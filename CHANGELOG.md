...
- User messages are now cross-relayed to all other platforms in the same `conversationLinks` conversation. When a message arrives on one platform it is forwarded to every other configured platform as `[platform] text` so all connected clients stay in sync in real time, not only after the AI replies.
