...
- Persona names passed to `executeSlashCommandsWithOptions` are now sanitized before use. SillyTavern's slash command runner supports pipe chaining (`|`), so an unsanitized name such as `Alice | /newchat` would have executed `/newchat` as a second command. Newlines carry the same risk. A new `sanitizePersonaName()` helper strips pipe characters and newlines and enforces a 200-character cap. Applied to all three call sites: `/persona`, `/mypersona`, and the auto-switch path in `handleUserMessage`.
